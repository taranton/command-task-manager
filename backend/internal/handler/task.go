...
func (h *TaskHandler) UpdateStatus(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.taskService.UpdateTaskStatus(r.Context(), id, input.Status); err != nil {
...
func (h *TaskHandler) UpdatePosition(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.taskService.UpdateTaskPosition(r.Context(), id, input.Position); err != nil {
		respondError(w, http.StatusInternalServerError, "failed to update position")
		return
	}

	// If status is also being updated (drag between columns)
	if input.Status != nil {
		status := model.Status(*input.Status)
		h.taskService.UpdateTaskStatus(r.Context(), id, status)
	}
	...
	respondJSON(w, http.StatusOK, map[string]interface{}{"id": id, "position": input.Position})
}
...
// unused but needed for uuid import
var _ = uuid.Nil
